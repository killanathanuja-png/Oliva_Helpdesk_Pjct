...
const FinanceApprovalsPage = () => {
  ...
  const fetchTickets = useCallback(async () => {
    try {
      ...
      // Show only Zenoti-Finance tickets (aom_finance type)
      setData(allTickets.filter((t) =>
        t.approvalType === "aom_finance"
      ));
    } catch {
      setData([]);
    } finally {
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold font-display">Finance Approvals</h1>
          <p className="text-xs text-muted-foreground">Zenoti-Finance requests approved by AOM, pending your review</p>
        </div>
      </div>
